...
class CheckEntities(Resource):
    def post(self):
        if request.is_json:
            ...
            combined_entities_result = ExtractEntities.combine_all_extracted_entities(extracted_gazetteer_entities, extracted_fuzzy_entities, extracted_spacy_model_entities, extracted_bert_model_entities)

            # return a json response
            return make_response(jsonify({'text': text, 'extractedGazetteerEntities': extracted_gazetteer_entities, 'extractedFuzzyEntities': extracted_fuzzy_entities, 'extractedEntitiesFromSpacyModel':extracted_spacy_model_entities, 'extractedEntitiesFromBertModel': extracted_bert_model_entities, 'combinedEntitiesResult': combined_entities_result}), 200)
        else:
